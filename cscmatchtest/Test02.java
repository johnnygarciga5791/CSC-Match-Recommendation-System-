...
public class Test02 extends TestCore {
    ...
    public void testMember() {
        ...
        String colonName = new String("no : allowed");
        int testYear = 1;
        String gotName;
        int gotYear;
        String errMsg;
        ...
        try {
            tc.newInstance(colonName, testYear);
            fail("constructor allowed name to be set to \"" + colonName + "\"");
        } catch (InvocationTargetException e1) {
            Throwable e2 = (Exception) e1.getCause();
            if (e2.getClass().equals(illegalArgumentExceptionClass.getClazz())) {
                // Setting name to "no : allowed" was blocked
            } else {
                unexpected(e2);
            }
        } catch (Exception e) {
        ...
        try {
            ...
            errMsg = (String) validateName.invoke(null, colonName);
            if (errMsg == null) {
                fail("validateName did not provide an error for \"" + colonName + "\"");
            }
        ...
        try {
            setName.invoke(mbr, colonName);
            fail("setName allowed name to be changed to \"" + colonName + "\"");
        } catch (InvocationTargetException e1) {
            Throwable e2 = (Exception) e1.getCause();
            if (!e2.getClass().equals(illegalArgumentExceptionClass.getClazz())) {
                unexpected(e2);
