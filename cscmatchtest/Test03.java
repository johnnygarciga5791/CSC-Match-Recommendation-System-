...
public class Test03 extends TestCore {
    ...
    public void testInterest() {
        ...
        String bangTopic = new String("no ! allowed");
        int testLevel = 1;
        String gotTopic;
        int gotLevel;
        String errMsg;
        ...
        try {
            tc.newInstance(bangTopic, testLevel);
            fail("constructor allowed topic to be set to \"" + bangTopic + "\"");
        } catch (InvocationTargetException e1) {
            Throwable e2 = (Exception) e1.getCause();
            if (e2.getClass().equals(illegalArgumentExceptionClass.getClazz())) {
                // Setting topic to "no ! allowed" was blocked
            } else {
                unexpected(e2);
            }
        } catch (Exception e) {
        ...
        try {
            ...
            errMsg = (String) validateTopic.invoke(bangTopic, "");
            if (errMsg == null) {
                fail("validateTopic did not provide an error for \"" + bangTopic + "\"");
            }
        ...
        try {
            ...
            fail("setTopic allowed topic to be changed to \"\"");
        } catch (InvocationTargetException e1) {
            Throwable e2 = (Exception) e1.getCause();
            if (!e2.getClass().equals(illegalArgumentExceptionClass.getClazz())) {
                unexpected(e2);
        ...
        try {
            setTopic.invoke(intr, bangTopic);
            fail("setTopic allowed topic to be changed to \"" + bangTopic + "\"");
        } catch (InvocationTargetException e1) {
            Throwable e2 = (Exception) e1.getCause();
            if (!e2.getClass().equals(illegalArgumentExceptionClass.getClazz())) {
                unexpected(e2);
