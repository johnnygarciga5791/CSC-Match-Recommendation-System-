...
abstract public class TestCore {
    ...
    final static protected ArrayList<Field> authorizedFields = new ArrayList<>();
    final static protected ArrayList<Method> authorizedMethods = new ArrayList<>();
    ...
    public void enforceStep1Restrictions(boolean testCompatibility) {
        TestClass[] allClasses = {
                cscMatchClass,
                menuClass,
                membershipClass,
                memberClass,
        ...
        try {
            ...
            if (totalFailures > 0) {
                fail("public methods and instance variable restrictions check skipped due to previous failures");
            }

            if (membershipClass.getClazz() != null) {
                requiredMethod(membershipClass, "toString", Modifier.PUBLIC, "String");
            }

            if (compatibilityClass.getClazz() != null) {
                requiredMethod(compatibilityClass, "toString", Modifier.PUBLIC, "String");
                requiredMethod(compatibilityClass, "compareTo", Modifier.PUBLIC, "int", compatibilityClass.getClazz());
            }

            for (TestClass tc : allClasses) {
                if (tc.getClazz() == null) {
                    continue;
                }
                if (tc == compatibilityClass && !testCompatibility) {
                    continue;
                }
                for (Field f : tc.getClazz().getDeclaredFields()) {
                    if (f.getName().equals("serialVersionUID")) {
                        continue;
                    }
                    if (!(authorizedFields.contains(f))) {
                        try {
                            fail("Unauthorized instance variable in " + tc.getName() + ": " + f.toGenericString());
                        } catch (TestExit e) {
                        }
                    }
                }
                for (Method m : tc.getClazz().getDeclaredMethods()) {
                    if (authorizedMethods.contains(m) || (m.getModifiers() & Modifier.PRIVATE) != 0) {
                        continue;
                    }
                    if (m.getName().equals("compareTo")) {
                        // compareTo has an odd behavior in which it appears twice: once in its normal
                        // form and again in a form that accepts an Object, likely due to some type
                        // erasure detail. If compareTo shows up in this form, ignore it.
                        try {
                            Method m2 = tc.getClazz().getDeclaredMethod(m.getName(), new Class[] { Object.class });
                            if (m.toGenericString().equals(m2.toGenericString())) {
                                continue;
                            }
                        } catch (Exception e) {
                        }
                    ...
                    try {
                        fail("Unauthorized non-private method in " + tc.getName() + ": " + m.toGenericString());
                    } catch (TestExit e) {
                    }
                }

                if (failed) {
                    fail("Unauthorized variable or method detected");
                }
            }
        } catch (TestExit e) {
        }
